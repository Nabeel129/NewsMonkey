  ...
  const getCountry = () => {
    ...
    for (let i = 0; i < country.length - 1; i++) {
      for (let j = 0; j < data.length - 1; j++) {
        if ((country[i].toUpperCase()) === data[j].code) {
          avalCountries[i] = data[j];
          document.getElementById('codes').innerHTML += `<li><Link className="dropdown-item" onClick={this.countryClick} to="/${data[j].code.toLowerCase()}">${data[j].name}</Link></li>`;
        }
      }
    }
  }
